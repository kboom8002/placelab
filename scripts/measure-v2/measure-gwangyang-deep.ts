#!/usr/bin/env npx tsx
// scripts/measure-v2/measure-yeosu-deep.ts
// 광양시 공격적 심화 프로브 42문항 × 3회 = 126슬롯 측정
...
const SYSTEM_KO = `당신은 한국 산업·관광·도시경제 전문 지식을 가진 AI 어시스턴트입니다. 사용자의 질문에 정확하게, 최신 정보를 기반으로 답해주세요. 답변은 3~5문장으로 핵심 정보를 간결하게 제공하세요. 출처가 있으면 URL도 함께 알려주세요.`;
const SYSTEM_EN = `You are an AI assistant with expertise in Korean industrial cities, steel industry, and tourism. Answer accurately based on the most up-to-date information. Provide concise answers in 3-5 sentences. Include source URLs if available.`;

const geminiKey = process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY;
if (!geminiKey) { console.error('\n❌ GEMINI_API_KEY 없음'); process.exit(1); }
const client = new GoogleGenAI({ apiKey: geminiKey });

const questionFile = path.resolve(__dirname, '../../docs/aeo-questions/gwangyang-deep.json');
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════');
  console.log('  광양시 공격적 AI 심화 진단 (Gemini + Search Grounding)');
  ...
  const measurementId = `m-${dateStr}-gwangyang-deep`;
  ...
  const sovMentionYeosu = sovQuestions.filter(r => /광양|gwangyang/i.test(r.response)).length;

  const output = {
    measurement: {
      id: measurementId, subject: '광양시', subject_type: 'local_gov', subject_code: '46170',
      method_version: 'v1.0-gwangyang-deep', ai_service: 'gemini', model: MODEL, search_grounding: true,
