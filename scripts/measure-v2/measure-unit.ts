// scripts/measure-v2/measure-unit.ts
// 범용 측정 스크립트: 지자체명 + 반복 횟수를 인자로 받음
// 실행: npx tsx scripts/measure-v2/measure-unit.ts --unit 증평군 --reps 5
...
const UNIT = unitIdx >= 0 ? args[unitIdx + 1] : '증평군';
const REPS = repsIdx >= 0 ? parseInt(args[repsIdx + 1]) : 1;

const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
const MODEL = 'gpt-5.6-luna';

function buildQuestions(unit: string) {
  return [
    ...
    { id: 'P3-04', persona: 'P3', category: '민원', body: `${unit} 군청 민원실 점심시간에 되는지, 주차요금 얼마야?` },
...
async function measureOne(q: ReturnType<typeof buildQuestions>[0], rep: number): Promise<Result> {
  ...
  try {
    ...
    console.log(`  ✓ ${q.id} R${rep} (${q.category}) — ${elapsed}ms`);
    return {
      id: q.id, persona: q.persona, category: q.category,
      question: q.body, rep,
    ...
    console.error(`  ✗ ${q.id} R${rep} (${q.category}) — ERROR: ${err.message}`);
    return {
      id: q.id, persona: q.persona, category: q.category,
      question: q.body, rep,
...
async function main() {
  const questions = buildQuestions(UNIT);
  const total = questions.length * REPS;

  console.log('═══════════════════════════════════════════════');
  console.log(`  kplacelab v2.0 측정`);
  console.log(`  단위: ${UNIT} | 모델: ${MODEL}`);
  console.log(`  문항: ${questions.length} | 반복: ${REPS}회 | 총: ${total}회`);
  console.log('═══════════════════════════════════════════════');
  ...
  for (let rep = 1; rep <= REPS; rep++) {
    ...
    for (let i = 0; i < questions.length; i += 5) {
      ...
      const batchResults = await Promise.all(batch.map(q => measureOne(q, rep)));
  ...
  const unitSlug = UNIT.replace(/\s/g, '');
  const outDir = path.resolve(__dirname, '../../docs');
  const outFile = path.join(outDir, `L2-${unitSlug}-quick-v2-raw.json`);
  fs.writeFileSync(outFile, JSON.stringify({
    measurement: {
      unit: UNIT, model: MODEL, method_version: 'v2.0',
      web_search: 'on (model default)', mode: REPS === 1 ? 'quick' : `quick-${REPS}rep`,
      reps: REPS, measured_on: new Date().toISOString().slice(0, 10),
      total_questions: questions.length, total_observations: results.length,
    },
    results,
  }, null, 2), 'utf-8');

  const errors = results.filter(r => r.response.startsWith('[ERROR]'));
  console.log(`\n✓ 측정 완료! ${results.length}건`);
  console.log(`  저장: ${outFile}`);
  console.log(`  성공: ${results.length - errors.length}건 / 에러: ${errors.length}건`);
}
